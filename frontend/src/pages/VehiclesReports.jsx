            ...
            <div className="bg-black px-6 py-4 flex justify-between items-center border-b-4 border-yellow-400 shadow-xl z-50">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-black text-lg tracking-tight uppercase">
                        Vehicle <span className="text-yellow-400">Inventory</span>
                    </h1>
                </div>
            </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 p-8 overflow-y-auto">
                    <div className="flex justify-between items-end mb-8">
                        <div>
                            <h2 className="text-3xl font-black uppercase tracking-tight">Vehicle Reports</h2>
                            <p className="text-gray-500 font-medium">Maintenance logs and registration history</p>
                        </div>
                        ...
                        <div className="divide-y divide-white/5">
                            {buses.map((bus) => (
                                <div key={bus.busId} className="grid grid-cols-5 items-center px-6 py-5 text-sm hover:bg-white/[0.02] transition-all">
                                    ...
                                    <div className="font-mono text-gray-500 text-[10px]">WCTM-{bus.busNumber}-DL</div>
                                    <div>
                                        {bus.tripActive ? (
                                            <div className="flex items-center gap-2">
                                                <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_green]"></span>
                                                <span className="text-[10px] text-green-400 font-black uppercase tracking-widest">Running</span>
                                            </div>
                                        ) : (
                                            <div className="flex items-center gap-2">
                                                <span className="w-2 h-2 bg-zinc-700 rounded-full"></span>
                                                <span className="text-[10px] text-gray-600 font-black uppercase tracking-widest">Stationary</span>
                                            </div>
                                        )}
                                    </div>
                                    <div className="text-right">
                                        <button 
                                            onClick={() => navigate(`/vehicle-report/${bus.busNumber}`)}
