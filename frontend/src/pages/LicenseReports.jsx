            ...
            <div className="bg-black px-6 py-4 flex justify-between items-center border-b-4 border-yellow-400 shadow-xl z-50">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-black text-lg tracking-tight uppercase">
                        License <span className="text-yellow-400">Compliance</span>
                    </h1>
                </div>
            </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 p-8 overflow-y-auto">
                    <div className="flex justify-between items-end mb-8">
                        <div>
                            <h2 className="text-3xl font-black uppercase tracking-tight">License Reports</h2>
                            <p className="text-gray-500 font-medium">Verify legal operation status for all pilots</p>
                        </div>
                        ...
                        <div className="divide-y divide-white/5">
                            {buses.map((bus) => (
                                <div key={bus.busId} className="grid grid-cols-5 items-center px-6 py-5 text-sm hover:bg-white/[0.02] transition-all">
                                    <div className="font-black text-white">BUS {bus.busNumber}</div>
                                    <div className="text-gray-300 font-bold">{bus.fullName}</div>
                                    <div className="font-mono text-gray-500 text-xs">DL-{bus.busNumber}X-2026</div>
                                    <div>
                                        <span className="bg-green-500/10 text-green-500 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-green-500/20 flex w-fit items-center gap-2">
                                            <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span> Valid
                                        </span>
                                    </div>
                                    <div className="text-right">
                                        <button 
                                            onClick={() => navigate(`/licence-report/${bus.busId}`)}
